...
(function () {
  ...
  var BLUEPRINTS = [
    ...
    { id: 'brunnen', name: 'Brunnen', cost: { stein: 4 },
      grid: ['.....', '.www.', '.www.', '.www.', '.....'] },
    { id: 'garten', name: 'Garten', cost: { holz: 5, stein: 2 },
      grid: ['.......', '.LLLLL.', '.LdddL.', '.LdddL.', '.LLLLL.'] },
    { id: 'bruecke', name: 'Br\u00fccke', cost: { holz: 8, stein: 4 },
      grid: ['LLLLLLLLL', 'ppppppppp', 'LLLLLLLLL'] },
    { id: 'leuchtturm', name: 'Leuchtturm', cost: { stein: 10, gold: 3, diamant: 1 },
      grid: ['sssss', 'sssss', 'sssss', 'sssss', 'sssss'] }
  ];
  ...
  // ----- Neue Aufgabentypen für starke Rechner -----
  function genSequence(diff) {
    var patterns;
    if (diff === 'profi') {
      patterns = [
        function () { var s = rnd(2, 5), st = rnd(2, 4); return { seq: [s, s + st, s + 2 * st, s + 3 * st], ans: s + 4 * st }; },
        function () { var s = rnd(2, 3); return { seq: [s, s * 2, s * 4, s * 8], ans: s * 16 }; },
        function () { var s = rnd(24, 44), st = rnd(3, 6); return { seq: [s, s - st, s - 2 * st, s - 3 * st], ans: s - 4 * st }; }
      ];
    } else {
      patterns = [
        function () { var s = rnd(1, 5), st = rnd(1, 3); return { seq: [s, s + st, s + 2 * st, s + 3 * st], ans: s + 4 * st }; },
        function () { var s = rnd(2, 6); return { seq: [s, s * 2, s * 3, s * 4], ans: s * 5 }; }
      ];
    }
    var p = pick(patterns)();
    return {
      type: 'sequence', seq: p.seq,
      question: 'Wie geht die Reihe weiter?',
      speak: 'Wie geht die Zahlenreihe weiter? ' + p.seq.join(', '),
      equation: p.seq.join('   ') + '   ?',
      correct: p.ans,
      options: makeOptions(p.ans, 0, 200, [-1, 1, -2, 2, (p.seq[1] - p.seq[0])]),
      kind: 'number'
    };
  }

  function genStory(diff) {
    var max = diff === 'profi' ? 20 : (diff === 'schwer' ? 15 : 10);
    var a = rnd(6, max), b = rnd(2, Math.max(2, Math.floor(a / 2))), c = rnd(1, 5);
    var scenarios = [
      { t: 'Du hast ' + a + ' Diamanten. Du gibst ' + b + ' weg und findest ' + c + '. Wie viele hast du jetzt?', ans: a - b + c },
      { t: 'Im Stall sind ' + a + ' Schweine. ' + b + ' laufen weg, dann kommen ' + c + ' dazu. Wie viele sind im Stall?', ans: a - b + c },
      { t: 'Du baust mit ' + a + ' Bl\u00f6cken. ' + c + ' fallen runter. Wie viele stehen noch?', ans: a - c },
      { t: 'Du findest ' + a + ' Goldbarren und ' + b + ' dazu. Wie viele sind es zusammen?', ans: a + b }
    ];
    var s = pick(scenarios);
    return {
      type: 'story', answer: s.ans,
      question: s.t,
      speak: s.t,
      equation: '',
      correct: s.ans,
      options: makeOptions(s.ans, 0, 60, [-1, 1, -2, 2, -b, b]),
      kind: 'number', isStory: true
    };
  }

  function genChain(diff) {
    var max = diff === 'profi' ? 15 : 10;
    var a = rnd(2, max), b = rnd(2, max), c = rnd(2, max);
    var ans, disp;
    if (Math.random() < 0.5) { ans = a + b + c; disp = a + ' + ' + b + ' + ' + c; }
    else { if (c > a + b) c = rnd(2, a + b); ans = a + b - c; disp = a + ' + ' + b + ' \u2212 ' + c; }
    return {
      type: 'chain', a: a, b: b, c: c,
      question: 'Rechne aus:',
      speak: 'Rechne aus: ' + disp.replace('\u2212', 'minus').replace(/\+/g, 'plus'),
      equation: disp + ' = ?',
      correct: ans,
      options: makeOptions(ans, 0, 60, [-1, 1, -2, 2, -c, c]),
      kind: 'number'
    };
  }
  ...
  var SESSION_PLANS = {
    ...
    mittel: ['count', 'add', 'add', 'sub', 'sub', 'double', 'sequence', 'compare'],
    schwer: ['add', 'sub', 'double', 'missing', 'mul', 'sequence', 'story', 'chain'],
    profi: ['add', 'sub', 'missing', 'mul', 'mul', 'sequence', 'story', 'chain']
  };
  ...
  function genByType(t, diff) {
    ...
    if (t === 'sequence') return genSequence(diff);
    if (t === 'story') return genStory(diff);
    if (t === 'chain') return genChain(diff);
    return genMul(diff);
  }
  ...
  function taskSignature(t) {
    ...
    if (t.type === 'sequence') return 'seq:' + (t.seq ? t.seq.join('_') : t.correct);
    if (t.type === 'story') return 'story:' + t.correct;
    if (t.type === 'chain') return 'chain:' + t.a + '_' + t.b + '_' + t.c;
  ...
  function renderTask() {
    ...
    if (t.type === 'count') {
      ...
      eq.textContent = t.equation;
    } else if (t.type === 'sequence' || t.type === 'chain') {
      eq.textContent = t.equation;
    } else if (t.type === 'story') {
      // Geschichte steht in der Frage; kein zusätzliches equation nötig
      eq.textContent = '';
  ...
  var BUILD3D_DATA = {
    ...
    water: 'data:image/png;base64,',
    poppy: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAcUlEQVQ4y2NgGAW0B28NdP7PVpL6T5bG/aqaYM0w9kU2TtI0lygzg2mSXQHSgIzJ8gLIZgU3tf9qUar/yQ5AsXYGBq9+DfIMEEhGNQDEJwuADCBZM7LftQu0wAaA+CSFB8iQaFczMA3CZDkf5hJ8BgAAxodHN8UEDfgAAAAASUVORK5CYII=',
    dandelion: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAUklEQVQ4y2NgGAXDHfx/4/8fhMnW/O+a5f+Pc1VJNwSkAaZxb5YS2CCyDCFbMwyI17D8B2GyNHr1a/wPXa0Lpsm2HWYACJPlEpABZHsDBIixGQCyM01OrS9sngAAAABJRU5ErkJggg==',
    bricks: 'data:image/png;base64,',
    mossy_cobble: 'data:image/png;base64,',
    stone_bricks: 'data:image/png;base64,',
    lantern: 'data:image/png;base64,',
    sandstone: 'data:image/png;base64,'
  };
  var BUILD3D_TEX = {
  ...
  var HOUSE_MODELS = {"starter":[[-1,0,-1,"grass_top"],[-1,-1,-1,"dirt"],[-1,0,0,"grass_top"],[-1,-1,0,"dirt"],[-1,0,1,"grass_top"],[-1,-1,1,"dirt"],[-1,0,2,"grass_top"],[-1,-1,2,"dirt"],[-1,0,3,"grass_top"],[-1,-1,3,"dirt"],[-1,0,4,"grass_top"],[-1,-1,4,"dirt"],[-1,0,5,"grass_top"],[-1,-1,5,"dirt"],[-1,0,6,"grass_top"],[-1,-1,6,"dirt"],[-1,0,7,"grass_top"],[-1,-1
"slab"],[5,4,0,"slab"],[5,4,6,"slab"],[6,4,0,"slab"],[6,4,1,"slab"],[6,4,2,"slab"],[6,4,3,"slab"],[6,4,4,"slab"],[6,4,5,"slab"],[6,4,6,"slab"],[3,5,3,"slab"],[0,4,0,"slab"],[0,4,1,"slab"],[0,4,2,"slab"],[0,4,3,"slab"],[0,4,4,"slab"],[0,4,5,"slab"],[0,4,6,"slab"],[1,4,0,"slab"],[1,4,6,"slab"],[2,4,0,"slab"],[2,4,6,"slab"],[3,4,0,"slab"],[3,4,6,"slab"],[4,4,0,"slab"],[4,4,6,"slab"],[5,4,0,"slab"],[5,4,6,"slab"],[6,4,0,"slab"],[6,4,1,"slab"],[6,4,2,"slab"],[6,4,3,"slab"],[6,4,4,"slab"],[6,4,5,"slab"],[6,4,6,"slab"],[1,5,1,"stone"],[1,5,2,"stone"],[1,5,3,"stone"],[1,5,4,"stone"],[1,5,5,"stone"],[2,5,1,"stone"],[2,5,5,"stone"],[3,5,1,"stone"],[3,5,5,"stone"],[4,5,1,"stone"],[4,5,5,"stone"],[5,5,1,"stone"],[5,5,2,"stone"],[5,5,3,"stone"],[5,5,4,"stone"],[5,5,5,"stone"],[2,6,2,"stone"],[2,6,3,"stone"],[2,6,4,"stone"],[3,6,2,"stone"],[3,6,4,"stone"],[4,6,2,"stone"],[4,6,3,"stone"],[4,6,4,"stone"],[3,7,3,"stone"],[1,5,1,"cobble"],[1,6,1,"cobble"],[5,1,-1,"glow"]],"stone":[[-1,0,-1,"grass_top"],[-1,-1,-1,"dirt"],[-1,0,0,"grass_top"],[-1,-1,0,"dirt"],[-1,0,1,"grass_top"],[-1,-1,1,"dirt"],[-1,0,2,"grass_top"],[-1,-1,2,"dirt"],[-1,0,3,"grass_top"],[-1,-1,3,"dirt"],[-1,0,4,"grass_top"],[-1,-1,4,"dirt"],[-1,0,5,"grass_top"],[-1,-1,5,"dirt"],[-1,0,6,"grass_top"],[-1,-1,6,"dirt"],[-1,0,7,"grass_top"],[-1,-1,7,"dirt"],[0,0,-1,"grass_top"],[0,-1,-1,"dirt"],[0,0,0,"grass_top"],[0,-1,0,"dirt"],[0,0,1,"grass_top"],[0,-1,1,"dirt"],[0,0,2,"grass_top"],[0,-1,2,"dirt"],[0,0,3,"grass_top"],[0,-1,3,"dirt"],[0,0,4,"grass_top"],[0,-1,4,"dirt"],[0,0,5,"grass_top"],[0,-1,5,"dirt"],[0,0,6,"grass_top"],[0,-1,6,"dirt"],[0,0,7,"grass_top"],[0,-1,7,"dirt"],[1,0,-1,"grass_top"],[1,-1,-1,"dirt"],[1,0,0,"grass_top"],[1,-1,0,"dirt"],[1,0,1,"grass_top"],[1,-1,1,"dirt"],[1,0,2,"grass_top"],[1,-1,2,"dirt"],[1,0,3,"grass_top"],[1,-1,3,"dirt"],[1,0,4,"grass_top"],[1,-1,4,"dirt"],[1,0,5,"grass_top"],[1,-1,5,"dirt"],[1,0,6,"grass_top"],[1,-1,6,"dirt"],[1,0,7,"grass_top"],[1,-1,7,"dirt"],[2
stone"],[0,1,2,"stone"],[0,1,3,"stone"],[0,1,4,"stone"],[0,1,5,"stone"],[0,1,6,"log"],[1,1,0,"stone"],[1,1,6,"stone"],[2,1,0,"stone"],[2,1,6,"stone"],[3,1,6,"stone"],[4,1,0,"stone"],[4,1,6,"stone"],[5,1,0,"stone"],[5,1,6,"stone"],[6,1,0,"log"],[6,1,1,"stone"],[6,1,2,"stone"],[6,1,3,"stone"],[6,1,4,"stone"],[6,1,5,"stone"],[6,1,6,"log"],[0,2,0,"log"],[0,2,1,"stone"],[0,2,2,"glass"],[0,2,3,"stone"],[0,2,4,"glass"],[0,2,5,"stone"],[0,2,6,"log"],[1,2,0,"stone"],[1,2,6,"stone"],[2,2,0,"glass"],[2,2,6,"glass"],[3,2,6,"stone"],[4,2,0,"glass"],[4,2,6,"glass"],[5,2,0,"stone"],[5,2,6,"stone"],[6,2,0,"log"],[6,2,1,"stone"],[6,2,2,"glass"],[6,2,3,"stone"],[6,2,4,"glass"],[6,2,5,"stone"],[6,2,6,"log"],[0,3,0,"log"],[0,3,1,"stone"],[0,3,2,"stone"],[0,3,3,"stone"],[0,3,4,"stone"],[0,3,5,"stone"],[0,3,6,"log"],[1,3,0,"stone"],[1,3,6,"stone"],[2,3,0,"stone"],[2,3,6,"stone"],[3,3,0,"stone"],[3,3,6,"stone"],[4,3,0,"stone"],[4,3,6,"stone"],[5,3,0,"stone"],[5,3,6,"stone"],[6,3,0,"log"],[6,3,1,"stone"],[6,3,2,"stone"],[6,3,3,"stone"],[6,3,4,"stone"],[6,3,5,"stone"],[6,3,6,"log"],[3,1,0,"door_low"],[3,2,0,"door_up"],[0,4,0,"slab"],[0,4,1,"slab"],[0,4,2,"slab"],[0,4,3,"slab"],[0,4,4,"slab"],[0,4,5,"slab"],[0,4,6,"slab"],[1,4,0,"slab"],[1,4,6,"slab"],[2,4,0,"slab"],[2,4,6,"slab"],[3,4,0,"slab"],[3,4,6,"slab"],[4,4,0,"slab"],[4,4,6,"slab"],[5,4,0,"slab"],[5,4,6,"slab"],[6,4,0,"slab"],[6,4,1,"slab"],[6,4,2,"slab"],[6,4,3,"slab"],[6,4,4,"slab"],[6,4,5,"slab"],[6,4,6,"slab"],[1,5,1,"cobble"],[1,5,2,"cobble"],[1,5,3,"cobble"],[1,5,4,"cobble"],[1,5,5,"cobble"],[2,5,1,"cobble"],[2,5,5,"cobble"],[3,5,1,"cobble"],[3,5,5,"cobble"],[4,5,1,"cobble"],[4,5,5,"cobble"],[5,5,1,"cobble"],[5,5,2,"cobble"],[5,5,3,"cobble"],[5,5,4,"cobble"],[5,5,5,"cobble"],[2,6,2,"cobble"],[2,6,3,"cobble"],[2,6,4,"cobble"],[3,6,2,"cobble"],[3,6,4,"cobble"],[4,6,2,"cobble"],[4,6,3,"cobble"],[4,6,4,"cobble"],[3,7,3,"cobble"],[5,1,-1,"glow"]],"tower":[[-1,0,-1,"grass_top"],[-1,-1,-1,"dirt"],[-1,0,0,"grass_top"],[-1,-1,0,"dirt"],[-1,0,1,"grass_top"],[-1,-1,1,"dirt"],[-1,0,2,"grass_top"],[-1,-1,2,"dirt"],[-1,0,3,"grass_top"],[-1,-1,3,"dirt"],[-1,0,4,"grass_top"],[-1,-1,4,"dirt"],[-1,0,5,"grass_top"],[-1,-1,5,"dirt"],[0,0,-1,"grass_top"],[0,-1,-1,"dirt"],[0,0,0,"grass_top"],[0,-1,0,"dirt"],[0,0,1,"grass_top"],[0,-1,1,"dirt"],[0,0,2,"grass_top"],[0,-1,2,"dirt"],[0,0,3,"grass_top"],[0,-1,3,"dirt"],[0,0,4,"grass_top"],[0,-1,4,"dirt"],[0,0,5,"grass_top"],[0,-1,5,"dirt"],[1,0,-1,"grass_top"],[1,-1,-1,"dirt"],[1,0,0,"grass_top"],[1,-1,0,"dirt"],[1,0,1,"grass_top"],[1,-1,1,"dirt"],[1,0,2,"grass_top"],[1,-1,2,"dirt"],[1,0,3,"grass_top"],[1,-1,3,"dirt"],[1,0,4,"grass_top"],[1,-1,4,"dirt"],[1,0,5,"grass_top"],[1,-1,5,"dirt"],[2,0,-1,"grass_top"],[2,-1,-1,"dirt"],[2,0,0,"grass_top"],[2,-1,0,"dirt"],[2,0,1,"grass_top"],[2,-1,1,"dirt"],[2,0,2,"grass_top"],[2,-1,2,"dirt"],[2,0,3,"grass_top"],[2,-1,3,"dirt"],[2,0,4,"grass_top"],[2,-1,4,"dirt"],[2,0,5,"grass_top"],[2,-1,5,"dirt"],[3,0,-1,"grass_top"],[3,-1,-1,"dirt"],[3,0,0,"grass_top"],[3,-1,0,"dirt"],[3,0,1,"grass_top"],[3,-1,1,"dirt"],[3,0,2,"grass_top"],[3,-1,2,"dirt"],[3,0,3,"grass_top"],[3,-1,3,"dirt"],[3,0,4,"grass_top"],[3,-1,4,"dirt"],[3,0,5,"grass_top"],[3,-1,5,"dirt"],[4,0,-1,"grass_top"],[4,-1,-1,"dirt"],[4,0,0,"grass_top"],[4,-1,0,"dirt"],[4,0,1,"grass_top"],[4,-1,1,"dirt"],[4,0,2,"grass_top"],[4,-1,2,"dirt"],[4,0,3,"grass_top"],[4,-1,3,"dirt"],[4,0,4,"grass_top"],[4,-1,4,"dirt"],[4,0,5,"grass_top"],[4,-1,5,"dirt"],[5,0,-1,"grass_top"],[5,-1,-1,"dirt"],[5,0,0,"grass_top"],[5,-1,0,"dirt"],[5,0,1,"grass_top"],[5,-1,1,"dirt"],[5,0,2,"grass_top"],[5,-1,2,"dirt"],[5,0,3,"grass_top"],[5,-1,3,"dirt"],[5,0,4,"grass_top"],[5,-1,4,"dirt"],[5,0,5,"grass_top"],[5,-1,5,"dirt"],[0,1,0,"log"],[0,1,1,"stone"],[0,1,2,"stone"],[0,1,3,"stone"],[0,1,4,"log"],[1,1,0,"stone"],[1,1,4,"stone"],[2,1,4,"stone"],[3,1,0,"stone"],[3,1,4,"stone"],[4,1,0,"log"],[4,1,1,"stone"],[4,1,2,"stone"],[4,1,3,"stone"],[4,1,4,"log"],[0,2,0,"log"],[0,2,1,"stone"],[0,2,2,"glass"],[0,2,3,"stone"],[0,2,4,"log"],[1,2,0,"stone"],[1,2,4,"stone"],[2,2,4,"glass"],[3,2,0,"stone"],[3,2,4,"stone"],[4,2,0,"log"],[4,2,1,"stone"],[4,2,2,"glass"],[4,2,3,"stone"],[4,2,4,"log"],[0,3,0,"log"],[0,3,1,"stone"],[0,3,2,"stone"],[0,3,3,"stone"],[0,3,4,"log"],[1,3,0,"stone"],[1,3,4,"stone"],[2,3,0,"stone"],[2,3,4,"stone"],[3,3,0,"stone"],[3,3,4,"stone"],[4,3,0,"log"],[4,3,1,"stone"],[4,3,2,"stone"],[4,3,3,"stone"],[4,3,4,"log"],[0,4,0,"log"],[0,4,1,"stone"],[0,4,2,"glass"],[0,4,3,"stone"],[0,4,4,"log"],[1,4,0,"stone"],[1,4,4,"stone"],[2,4,0,"glass"],[2,4,4,"glass"],[3,4,0,"stone"],[3,4,4,"stone"],[4,4,0,"log"],[4,4,1,"stone"],[4,4,2,"glass"],[4,4,3,"stone"],[4,4,4,"log"],[0,5,0,"log"],[0,5,1,"stone"],[0,5,2,"stone"],[0,5,3,"stone"],[0,5,4,"log"],[1,5,0,"stone"],[1,5,4,"stone"],[2,5,0,"stone"],[2,5,4,"stone"],[3,5,0,"stone"],[3,5,4,"stone"],[4,5,0,"log"],[4,5,1,"stone"],[4,5,2,"stone"],[4,5,3,"stone"],[4,5,4,"log"],[2,1,0,"door_low"],[2,2,0,"door_up"],[0,6,0,"stone"],[0,6,1,"stone"],[0,6,2,"stone"],[0,6,3,"stone"],[0,6,4,"stone"],[1,6,0,"stone"],[1,6,4,"stone"],[2,6,0,"stone"],[2,6,4,"stone"],[3,6,0,"stone"],[3,6,4,"stone"],[4,6,0,"stone"],[4,6,1,"stone"],[4,6,2,"stone"],[4,6,3,"stone"],[4,6,4,"stone"],[2,6,2,"glow"]],"porch":[[-2,0,-2,"grass_top"],[-2,-1,-2,"dirt"],[-2,0,-1,"grass_top"],[-2,-1,-1,"dirt"],[-2,0,0,"grass_top"],[-2,-1,0,"dirt"],[-2,0,1,"grass_top"],[-2,-1,1,"dirt"],[-2,0,2,"grass_top"],[-2,-1,2,"dirt"],[-2,0,3,"grass_top"],[-2,-1,3,"dirt"],[-2,0,4,"grass_top"],[-2,-1,4,"dirt"],[-2,0,5,"grass_top"],[-2,-1,5,"dirt"],[-2,0,6,"grass_top"],[-2,-1,6,"dirt"],[-2,0,7,"grass_top"],[-2,-1,7,"dirt"],[-2,0,8,"grass_top"],[-2,-1,8,"dirt"],[-1,0,-2,"grass_top"],[-1,-1,-2,"dirt"],[-1,0,-1,"grass_top"],[-1,-1,-1,"dirt"],[-1,0,0,"grass_top"],[-1,-1,0,"dirt"],[-1,0,1,"grass_top"],[-1,-1,1,"dirt"],[-1,0,2,"grass_top"],[-1,-1,2,"dirt"],[-1,0,3,"grass_top"],[-1,-1,3,"dirt"],[-1,0,4,"grass_top"],[-1,-1,4,"dirt"],[-1,0,5,"grass_top"],[-1,-1,5,"dirt"],[-1,0,6,"grass_top"],[-1,-1,6,"dirt"],[-1,0,7,"grass_top"],[-1,-1,7,"dirt"],[-1,0,8,"grass_top"],[-1,-1,8,"dirt"],[0,0,-2,"grass_top"],[0,-1,-2,"dirt"],[0,0,-1,"grass_top"],[0,-1,-1,"dirt"],[0,0,0,"grass_top"],[0,-1,0,"dirt"],[0,0,1,"grass_top"],[0,-1,1,"dirt"],[0,0,2,"grass_top"],[0,-1,2,"dirt"],[0,0,3,"grass_top"],[0,-1,3,"dirt"],[0,0,4,"grass_top"],[0,-1,4,"dirt"],[0,0,5,"grass_top"],[0,-1,5,"dirt"],[0,0,6,"grass_top"],[0,-1,6,"dirt"],[0,0,7,"grass_top"],[0,-1,7,"dirt"],[0,0,8,"grass_top"],[0,-1,8,"dirt"],[1,0,-2,"grass_top"],[1,-1,-2,"dirt"],[1,0,-1,"grass_top"],[1,-1,-1,"dirt"],[1,0,0,"grass_top"],[1,-1,0,"dirt"],[1,0,1,"grass_top"],[1,-1,1,"dirt"],[1,0,2,"grass_top"],[1,-1,2,"dirt"],[1,0,3,"grass_top"],[1,-1,3,"dirt"],[1,0,4,"grass_top"],[1,-1,4,"dirt"],[1,0,5,"grass_top"],[1,-1,5,"dirt"],[1,0,6,"grass_top"],[1,-1,6,"dirt"],[1,0,7,"grass_top"],[1,-1,7,"dirt"],[1,0,8,"grass_top"],[1,-1,8,"dirt"],[2,0,-2,"grass_top"],[2,-1,-2,"dirt"],[2,0,-1,"grass_top"],[2,-1,-1,"dirt"],[2,0,0,"grass_top"],[2,-1,0,"dirt"],[2,0,1,"grass_top"],[2,-1,1,"dirt"],[2,0,2,"grass_top"],[2,-1,2,"dirt"],[2,0,3,"grass_top"],[2,-1,3,"dirt"],[2,0,4,"grass_top"],[2,-1,4,"dirt"],[2,0,5,"grass_top"],[2,-1,5,"dirt"],[2,0,6,"grass_top"],[2,-1,6,"dirt"],[2,0,7,"grass_top"],[2,-1,7,"dirt"],[2,0,8,"grass_top"],[2,-1,8,"dirt"],[3,0,-2,"grass_top"],[3,-1,-2,"dirt"],[3,0,-1,"grass_top"],[3,-1,-1,"dirt"],[3,0,0,"grass_top"],[3,-1,0,"dirt"],[3,0,1,"grass_top"],[3,-1,1,"dirt"],[3,0,2,"grass_top"],[3,-1,2,"dirt"],[3,0,3,"grass_top"],[3,-1,3,"dirt"],[3,0,4,"grass_top"],[3,-1,4,"dirt"],[3,0,5,"grass_top"],[3,-1,5,"dirt"],[3,0,6,"grass_top"],[3,-1,6,"dirt"],[3,0,7,"grass_top"],[3,-1,7,"dirt"],[3,0,8,"grass_top"],[3,-1,8,"dirt"],[4,0,-2,"grass_top"],[4,-1,-2,"dirt"],[4,0,-1,"grass_top"],[4,-1,-1,"dirt"],[4,0,0,"grass_top"],[4,-1,0,"dirt"],[4,0,1,"grass_top"],[4,-1,1,"dirt"],[4,0,2,"grass_top"],[4,-1,2,"dirt"],[4,0,3,"grass_top"],[4,-1,3,"dirt"],[4,0,4,"grass_top"],[4,-1,4,"dirt"],[4,0,5,"grass_top"],[4,-1,5,"dirt"],[4,0,6,"grass_top"],[4,-1,6,"dirt"],[4,0,7,"grass_top"],[4,-1,7,"dirt"],[4,0,8,"grass_top"],[4,-1,8,"dirt"],[5,0,-2,"grass_top"],[5,-1,-2,"dirt"],[5,0,-1,"grass_top"],[5,-1,-1,"dirt"],[5,0,0,"grass_top"],[5,-1,0,"dirt"],[5,0,1,"grass_top"],[5,-1,1,"dirt"],[5,0,2,"grass_top"],[5,-1,2,"dirt"],[5,0,3,"grass_top"],[5,-1,3,"dirt"],[5,0,4,"grass_top"],[5,-1,4,"dirt"],[5,0,5,"grass_top"],[5,-1,5,"dirt"],[5,0,6,"grass_top"],[5,-1,6,"dirt"],[5,0,7,"grass_top"],[5,-1,7,"dirt"],[5,0,8,"grass_top"],[5,-1,8,"dirt"],[6,0,-2,"grass_top"],[6,-1,-2,"dirt"],[6,0,-1,"grass_top"],[6,-1,-1,"dirt"],[6,0,0,"grass_top"],[6,-1,0,"dirt"],[6,0,1,"grass_top"],[6,-1,1,"dirt"],[6,0,2,"grass_top"],[6,-1,2,"dirt"],[6,0,3,"grass_top"],[6,-1,3,"dirt"],[6,0,4,"grass_top"],[6,-1,4,"dirt"],[6,0,5,"grass_top"],[6,-1,5,"dirt"],[6,0,6,"grass_top"],[6,-1,6,"dirt"],[6,0,7,"grass_top"],[6,-1,7,"dirt"],[6,0,8,"grass_top"],[6,-1,8,"dirt"],[7,0,-2,"grass_top"],[7,-1,-2,"dirt"],[7,0,-1,"grass_top"],[7,-1,-1,"dirt"],[7,0,0,"grass_top"],[7,-1,0,"dirt"],[7,0,1,"grass_top"],[7,-1,1,"dirt"],[7,0,2,"grass_top"],[7,-1,2,"dirt"],[7,0,3,"grass_top"],[7,-1,3,"dirt"],[7,0,4,"grass_top"],[7,-1,4,"dirt"],[7,0,5,"grass_top"],[7,-1,5,"dirt"],[7,0,6,"grass_top"],[7,-1,6,"dirt"],[7,0,7,"grass_top"],[7,-1,7,"dirt"],[7,0,8,"grass_top"],[7,-1,8,"dirt"],[8,0,-2,"grass_top"],[8,-1,-2,"dirt"],[8,0,-1,"grass_top"],[8,-1,-1,"dirt"],[8,0,0,"grass_top"],[8,-1,0,"dirt"],[8,0,1,"grass_top"],[8,-1,1,"dirt"],[8,0,2,"grass_top"],[8,-1,2,"dirt"],[8,0,3,"grass_top"],[8,-1,3,"dirt"],[8,0,4,"grass_top"],[8,-1,4,"dirt"],[8,0,5,"grass_top"],[8,-1,5,"dirt"],[8,0,6,"grass_top"],[8,-1,6,"dirt"],[8,0,7,"grass_top"],[8,-1,7,"dirt"],[8,0,8,"grass_top"],[8,-1,8,"dirt"],[0,1,2,"log"],[0,1,3,"planks"],[0,1,4,"planks"],[0,1,5,"planks"],[0,1,6,"log"],[1,1,2,"planks"],[1,1,6,"planks"],[2,1,2,"planks"],[2,1,6,"planks"],[3,1,6,"planks"],[4,1,2,"planks"],[4,1,6,"planks"],[5,1,2,"planks"],[5,1,6,"planks"],[6,1,2,"log"],[6,1,3,"planks"],[6,1,4,"planks"],[6,1,5
log"],[1,1,0,"log"],[1,1,6,"log"],[2,1,0,"log"],[2,1,6,"log"],[3,1,0,"log"],[3,1,6,"log"],[4,1,0,"log"],[4,1,6,"log"],[5,1,0,"log"],[5,1,6,"log"],[6,1,0,"log"],[6,1,1,"log"],[6,1,2,"log"],[6,1,3,"log"],[6,1,4,"log"],[6,1,5,"log"],[6,1,6,"log"],[1,1,1,"dirt"],[1,1,2,"dirt"],[1,1,3,"dirt"],[1,1,4,"dirt"],[1,1,5,"dirt"],[2,1,1,"dirt"],[2,1,2,"dirt"],[2,1,3,"dirt"],[2,1,4,"dirt"],[2,1,5,"dirt"],[3,1,1,"dirt"],[3,1,2,"dirt"],[3,1,3,"dirt"],[3,1,4,"dirt"],[3,1,5,"dirt"],[4,1,1,"dirt"],[4,1,2,"dirt"],[4,1,3,"dirt"],[4,1,4,"dirt"],[4,1,5,"dirt"],[5,1,1,"dirt"],[5,1,2,"dirt"],[5,1,3,"dirt"],[5,1,4,"dirt"],[5,1,5,"dirt"],[2,2,2,"poppy"],[4,2,2,"dandelion"],[2,2,4,"dandelion"],[4,2,4,"poppy"],[3,2,3,"leaves"],[0,0,3,"path"],[1,0,3,"path"],[2,0,3,"path"],[3,0,3,"path"],[4,0,3,"path"],[5,0,3,"path"],[6,0,3,"path"]],"bruecke":[[0,0,0,"water"],[0,0,1,"water"],[0,0,2,"water"],[1,0,0,"water"],[1,0,1,"water"],[1,0,2,"water"],[2,0,0,"water"],[2,0,1,"water"],[2,0,2,"water"],[3,0,0,"water"],[3,0,1,"water"],[3,0,2,"water"],[4,0,0,"water"],[4,0,1,"water"],[4,0,2,"water"],[5,0,0,"water"],[5,0,1,"water"],[5,0,2,"water"],[6,0,0,"water"],[6,0,1,"water"],[6,0,2,"water"],[7,0,0,"water"],[7,0,1,"water"],[7,0,2,"water"],[8,0,0,"water"],[8,0,1,"water"],[8,0,2,"water"],[0,2,0,"planks"],[0,2,1,"planks"],[0,2,2,"planks"],[0,1,0,"log"],[0,1,2,"log"],[1,2,0,"planks"],[1,2,1,"planks"],[1,2,2,"planks"],[2,2,0,"planks"],[2,2,1,"planks"],[2,2,2,"planks"],[3,3,0,"planks"],[3,3,1,"planks"],[3,3,2,"planks"],[4,3,0,"planks"],[4,3,1,"planks"],[4,3,2,"planks"],[5,3,0,"planks"],[5,3,1,"planks"],[5,3,2,"planks"],[6,2,0,"planks"],[6,2,1,"planks"],[6,2,2,"planks"],[7,2,0,"planks"],[7,2,1,"planks"],[7,2,2,"planks"],[8,2,0,"planks"],[8,2,1,"planks"],[8,2,2,"planks"],[8,1,0,"log"],[8,1,2,"log"],[0,3,0,"log"],[0,3,2,"log"],[1,3,0,"log"],[1,3,2,"log"],[2,3,0,"log"],[2,3,2,"log"],[3,4,0,"log"],[3,4,2,"log"],[4,4,0,"log"],[4,4,2,"log"],[5,4,0,"log"],[5,4,2,"log"],[6,3,0,"log"],[6,3,2,"log"],[7,3,0,"log"],[7,3,2,"log"],[8,3,0,"log"],[8,3,2,"log"]],"leuchtturm":[[
  var MOB_MODELS = {"pig":[{"name":"body","c":[0.0,0.75,0.0],"s":[0.625,0.5,1.0],"box":[10,8,16],"uv":[28,8],"tex":"skin_pig","texSize":[64,32],"uvbox":[10,16,8]},{"name":"head","c":[0.0,0.75,-0.75],"s":[0.5,0.5,0.5],"box":[8,8,8],"uv":[0,0],"tex":"skin_pig","texSize":[64,32],"uvbox":[8,8,8]},{"name":"leg","c":[-0.1875,0.1875,0.3125],"s":[0.25,0.375,0.25],"box":[4,6,4],"uv":[0,16],"tex":"skin_pig","texSize":[64,32],"uvbox":[4,6,4]},{"name":"leg","c":[0.1875,0.1875,0.3125],"s":[0.25,0.375,0.25],"box":[4,6,4],"uv":[0,16],"tex":"skin_pig","texSize":[64,32],"uvbox":[4,6,4]},{"name":"leg","c":[-0.1875,0.1875,-0.3125],"s":[0.25,0.375,0.25],"box":[4,6,4],"uv":[0,16],"tex":"skin_pig","texSize":[64,32],"uvbox":[4,6,4]},{"name":"leg","c":[0.1875,0.1875,-0.3125],"s":[0.25,0.375,0.25],"box":[4,6,4],"uv":[0,16],"tex":"skin_pig","texSize":[64,32],"uvbox":[4,6,4]}],"chicken":[{"name":"body","c":[0.0,0.5,0.0],"s":[0.375,0.375,0.5],"box":[6,6,8],"uv":[0,9],"uvbox":[6,8,6],"tex":"skin_chicken","texSize":[64,32]},{"name":"head","c":[0.0,0.8125,-0.25],"s":[0.25,0.375,0.1875],"box":[4,6,3],"uv":[0,0],"uvbox":[4,6,3],"tex":"skin_chicken","texSize":[64,32]},{"name":"beak","c":[0.0,0.8125,-0.375],"s":[0.25,0.125,0.125],"box":[4,2,2],"uv":[14,0],"uvbox":[4,2,2],"tex":"skin_chicken","texSize":[64,32]},{"name":"wing_l","c":[-0.1875,0.5625,0.0],"s":[0.0625,0.25,0.375],"box":[1,4,6],"uv":[24,13],"uvbox":[1,4,6],"tex":"skin_chicken","texSize":[64,32]},{"name":"wing_r","c":[0.1875,0.5625,0.0],"s":[0.0625,0.25,0.375],"box":[1,4,6],"uv":[24,13],"uvbox":[1,4,6],"tex":"skin_chicken","texSize":[64,32]},{"name":"leg_l","c":[-0.125,0.125,0.0],"s":[0.1875,0.3125,0.1875],"box":[3,5,3],"uv":[26,0],"uvbox":[3,5,3],"tex":"skin_chicken","texSize":[64,32]},{"name":"leg_r","c":[0.125,0.125,0.0],"s":[0.1875,0.3125,0.1875],"box":[3,5,3],"uv":[26,0],"uvbox":[3,5,3],"tex":"skin_chicken","texSize":[64,32]}],"cat":[{"name":"body","c":[0.0,0.3125,0.0625],"s":[0.375,0.3125,0.75],"box":[6,5,12],"uv":[20,0],"uvbox":[6,12,5],"tex":"skin_cat","texSize":[64,32]},{"name":"head","c":[0.0,0.375,-0.375],"s":[0.3125,0.25,0.3125],"box":[5,4,5],"uv":[0,0],"uvbox":[5,4,5],"tex":"skin_cat","texSize":[64,32]},{"name":"tail","c":[0.0,0.3125,0.4375],"s":[0.125,0.5625,0.125],"box":[2,9,2],"uv":[0,15],"uvbox":[2,9,2],"tex":"skin_cat","texSize":[64,32]},{"name":"leg_fl","c":[-0.09375,0.125,-0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[8,13],"uvbox":[2,6,2],"tex":"skin_cat","texSize":[64,32]},{"name":"leg_fr","c":[0.09375,0.125,-0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[8,13],"uvbox":[2,6,2],"tex":"skin_cat","texSize":[64,32]},{"name":"leg_bl","c":[-0.09375,0.125,0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[8,13],"uvbox":[2,6,2],"tex":"skin_cat","texSize":[64,32]},{"name":"leg_br","c":[0.09375,0.125,0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[8,13],"uvbox":[2,6,2],"tex":"skin_cat","texSize":[64,32]}],"wolf":[{"name":"body","c":[0.0,0.5,0.0625],"s":[0.5,0.375,0.75],"box":[8,6,12],"uv":[18,14],"uvbox":[6,9,6],"tex":"skin_wolf","texSize":[64,32]},{"name":"head","c":[0.0,0.625,-0.4375],"s":[0.375,0.375,0.375],"box":[6,6,6],"uv":[0,0],"uvbox":[6,6,6],"tex":"skin_wolf","texSize":[64,32]},{"name":"tail","c":[0.0,0.5625,0.5],"s":[0.125,0.5,0.125],"box":[2,8,2],"uv":[9,18],"uvbox":[2,8,2],"tex":"skin_wolf","texSize":[64,32]},{"name":"leg_fl","c":[-0.125,0.1875,-0.25],"s":[0.125,0.5,0.125],"box":[2,8,2],"uv":[0,18],"uvbox":[2,8,2],"tex":"skin_wolf","texSize":[64,32]},{"name":"leg_fr","c":[0.125,0.1875,-0.25],"s":[0.125,0.5,0.125],"box":[2,8,2],"uv":[0,18],"uvbox":[2,8,2],"tex":"skin_wolf","texSize":[64,32]},{"name":"leg_bl","c":[-0.125,0.1875,0.25],"s":[0.125,0.5,0.125],"box":[2,8,2],"uv":[0,18],"uvbox":[2,8,2],"tex":"skin_wolf","texSize":[64,32]},{"name":"leg_br","c":[0.125,0.1875,0.25],"s":[0.125,0.5,0.125],"box":[2,8,2],"uv":[0,18],"uvbox":[2,8,2],"tex":"skin_wolf","texSize":[64,32]}],"fox":[{"name":"body","c":[0.0,0.375,0.0625],"s":[0.375,0.3125,0.75],"box":[6,5,12],"uv":[24,14],"uvbox":[6,6,12],"tex":"skin_fox","texSize":[64,32]},{"name":"head","c":[0.0,0.5,-0.4375],"s":[0.5,0.375,0.375],"box":[8,6,6],"uv":[1,5],"uvbox":[8,6,6],"tex":"skin_fox","texSize":[64,32]},{"name":"tail","c":[0.0,0.375,0.5625],"s":[0.25,0.25,0.5625],"box":[4,4,9],"uv":[30,0],"uvbox":[4,4,9],"tex":"skin_fox","texSize":[64,32]},{"name":"leg_fl","c":[-0.09375,0.125,-0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[13,24],"uvbox":[2,6,2],"tex":"skin_fox","texSize":[64,32]},{"name":"leg_fr","c":[0.09375,0.125,-0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[13,24],"uvbox":[2,6,2],"tex":"skin_fox","texSize":[64,32]},{"name":"leg_bl","c":[-0.09375,0.125,0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[13,24],"uvbox":[2,6,2],"tex":"skin_fox","texSize":[64,32]},{"name":"leg_br","c":[0.09375,0.125,0.25],"s":[0.125,0.375,0.125],"box":[2,6,2],"uv":[13,24],"uvbox":[2,6,2],"tex":"skin_fox","texSize":[64,32]}],"axolotl":[{"name":"body","c":[0.0,0.25,0.0625],"s":[0.3125,0.25,0.5625],"box":[5,4,9],"uv":[0,11],"uvbox":[5,4,9],"tex":"skin_axolotl","texSize":[64,64]},{"name":"head","c":[0.0,0.3125,-0.3125],"s":[0.3125,0.3125,0.3125],"box":[5,5,5],"uv":[0,1],"uvbox":[5,5,5],"tex":"skin_axolotl","texSize":[64,64]},{"name":"tail","c":[0.0,0.25,0.4375],"s":[0.1875,0.3125,0.5],"box":[3,5,8],"uv":[2,19],"uvbox":[3,5,8],"tex":"skin_axolotl","texSize":[64,64]},{"name":"leg_fl","c":[-0.125,0.0625,-0.125],"s":[0.125,0.1875,0.125],"box":[2,3,2],"uv":[2,13],"uvbox":[2,3,2],"tex":"skin_axolotl","texSize":[64,64]},{"name":"leg_fr","c":[0.125,0.0625,-0.125],"s":[0.125,0.1875,0.125],"box":[2,3,2],"uv":[2,13],"uvbox":[2,3,2],"tex":"skin_axolotl","texSize":[64,64]},{"name":"leg_bl","c":[-0.125,0.0625,0.1875],"s":[0.125,0.1875,0.125],"box":[2,3,2],"uv":[2,13],"uvbox":[2,3,2],"tex":"skin_axolotl","texSize":[64,64]},{"name":"leg_br","c":[0.125,0.0625,0.1875],"s":[0.125,0.1875,0.125],"box":[2,3,2],"uv":[2,13],"uvbox":[2,3,2],"tex":"skin_axolotl","texSize":[64,64]}]};
  ...
  function build3dInit(guided) {
    ...
    var FALLBACK = {
      ...
      water: 0x3a6fd2, poppy: 0xd02a1e, dandelion: 0xe8d23a, bricks: 0x9a564a,
      mossy_cobble: 0x6f7a5a, stone_bricks: 0x8a8a8a, lantern: 0xffcf6e, sandstone: 0xe0d8b0,
  ...
  function buildMob(THREE, parts, mobMat, scale) {
    ...
    parts.forEach(function (p) {
      ...
      var ub = p.uvbox || p.box;
      var UW = ub[0], UH = ub[1], UD = ub[2];
      ...
      // Echte Minecraft Box-UV-Konvention mit UV-Maßen (UW,UH,UD):
      var rects = {
        px: [uo,                vo + UD, UD, UH], // +x = east
        nx: [uo + UD + UW,      vo + UD, UD, UH], // -x = west
        py: [uo + UD,           vo,      UW, UD], // +y = up
        ny: [uo + UD + UW,      vo,      UW, UD], // -y = down
        pz: [uo + 2 * UD + UW,  vo + UD, UW, UH], // +z = south (hinten)
        nz: [uo + UD,           vo + UD, UW, UH]  // -z = north (Gesicht)
      };
      var order = ['px', 'nx', 'py', 'ny', 'pz', 'nz'];
      var uvAttr = geo.attributes.uv;
      order.forEach(function (fk, fi) {
        var f = rects[fk];
        var u0 = f[0] / tw, u1 = (f[0] + f[2]) / tw;
        var v0 = 1 - f[1] / th, v1 = 1 - (f[1] + f[3]) / th;
        var base = fi * 4;
        // BoxGeometry-UV-Eckenreihenfolge: oben-links, oben-rechts, unten-links, unten-rechts
